...
void gfx_load_players (int sx, int sy) {
  ...
  for (i = 0; i < gfx.player_gfx_count; i++) {
    ...
    if (tmpimage == NULL) {
      ...
      gfx.players[i].ani.image = SDL_ConvertSurfaceFormat(tmpimage1, SDL_GetWindowPixelFormat(gfx.window), 0);
  ...
  getRGBpixel (tmpimage1, 0, 0, &r, &g, &b);
  SDL_SetColorKey (tmpimage1, SDL_TRUE, SDL_MapRGB (tmpimage1->format, r, g, b));
  gfx.dead.image = SDL_ConvertSurfaceFormat(tmpimage1, SDL_GetWindowPixelFormat(gfx.window), 0);
  SDL_FreeSurface (tmpimage1);
  SDL_FreeSurface (tmpimage);
  ...
  getRGBpixel (tmpimage1, 0, 0, &r, &g, &b);
  SDL_SetColorKey (tmpimage1, SDL_TRUE, SDL_MapRGB (tmpimage1->format, r, g, b));
  gfx.ill.image = SDL_ConvertSurfaceFormat(tmpimage1, SDL_GetWindowPixelFormat(gfx.window), 0);
  SDL_FreeSurface (tmpimage);
  SDL_FreeSurface (tmpimage1);
...
void gfx_init (void) {
  ...
  gfx.window=SDL_CreateWindow("bomberguy", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, gfx.res.x, gfx.res.y ,flags);
  ...
  gfx.renderer = SDL_CreateRenderer(gfx.screen, -1, SDL_RENDERER_ACCELERATED|SDL_RENDERER_SOFTWARE);
...
void gfx_loaddata () {
  ...
  gfx.logo = SDL_ConvertSurfaceFormat(tmpimage1, SDL_GetWindowPixelFormat(gfx.window), 0);
  SDL_FreeSurface (tmpimage1);
  ...
  for (j = 0; j < 3; j++) 
	...
	exit (1);
      ...
      menubuttonimages[j][i] = SDL_ConvertSurfaceFormat(tmpimage, SDL_GetWindowPixelFormat(gfx.window), 0);
      SDL_FreeSurface (tmpimage);
    }
  ...
  for (j = 0; j < 2; j++) 
	...
	exit (1);
      ...
      SDL_SetColorKey (tmpimage, SDL_TRUE, SDL_MapRGB (tmpimage->format, 255, 255, 255));
      menuentryimages[j][i] = SDL_ConvertSurfaceFormat(tmpimage, SDL_GetWindowPixelFormat(gfx.window), 0);
      SDL_FreeSurface (tmpimage);
    }
  ...
  for (j = 0; j < 2; j++) 
	...
	exit (1);
      ...
      SDL_SetColorKey (tmpimage, SDL_TRUE, SDL_MapRGB (tmpimage->format, 255, 255, 255));
      menulistimages[j][i] = SDL_ConvertSurfaceFormat(tmpimage, SDL_GetWindowPixelFormat(gfx.window), 0);
      SDL_FreeSurface (tmpimage);
    }
...
static void gfx_load_menusmall_players () {
  ...
  for (i = 0; i < gfx.player_gfx_count; i++) {
	...
	gfx.players[i].small_image = SDL_ConvertSurfaceFormat(tmpimg, SDL_GetWindowPixelFormat(gfx.window), 0);
	...
	getRGBpixel (tmpimg, 0, 0, &r, &g, &b);
	SDL_SetColorKey (tmpimg, SDL_TRUE, SDL_MapRGB (tmpimg->format, r, g, b));
	gfx.players[i].menu_image = SDL_ConvertSurfaceFormat(tmpimg, SDL_GetWindowPixelFormat(gfx.window), 0);
	SDL_FreeSurface (tmpimg);
      }
      SDL_FreeSurface (img);
    }
...
static void gfx_load_fieldtype_menu () {	
  ...
  for (ft = 0; ft < FT_max; ft++) if (ft != FT_mixed) {
	...
	SDL_FreeSurface (tmpimg);
	background = SDL_ConvertSurfaceFormat(orgimg, SDL_GetWindowPixelFormat(gfx.window), 0); 
	SDL_FreeSurface (orgimg);
      }
	...
	exit (1);
      ...
      tmpimg = SDL_ConvertSurfaceFormat(orgimg, SDL_GetWindowPixelFormat(gfx.window), 0);
      SDL_FreeSurface (orgimg);
...
void shade_pixel (SDL_Surface * s, int x, int y, int c) {
  Uint32 p;
  Uint8 r,
    g,
    b;
...
void draw_shadefield (SDL_Surface * s, SDL_Rect * rec, int c) {
  int x1,
    x,
    x2,
    y1,
